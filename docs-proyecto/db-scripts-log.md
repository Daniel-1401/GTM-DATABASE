...
Alcance: diseño físico propuesto para SQL Server 2017. Ningún script de esta
tabla ha sido ejecutado contra una instancia. Las migraciones `001..008` aún no
han sido aplicadas; por ello la auditoría DB-NUCLEO-002 corrigió esos archivos
en sitio. Una vez aplicada una versión, cualquier cambio posterior deberá
entregarse como una migración nueva y no reescribir el historial.
...
| 007_crear_horarios_laborales_y_vigencias | 2026-09-04T08:00:21-05:00 | `rrhh.HorarioLaboral`, `rrhh.VigenciaHorario` | Mantener códigos de horario y asignarlos por relación laboral y día, permitiendo repeticiones. | `db/migrations/nucleo/007_crear_horarios_laborales_y_vigencias.sql` |
...
| 012_crear_idempotencia_y_auditoria_alimentacion | 2026-09-04T12:00:00-05:00 | RegistroIdempotencia, EventoAuditoria | Evitar efectos duplicados y conservar auditoría minimizada. | `db/migrations/alimentacion/012_crear_idempotencia_y_auditoria_alimentacion.sql` |
| aplicar_migraciones_gtm_alimentacion | 2026-09-04T12:00:00-05:00 | Migraciones `001..012` y `017` | Construir núcleo y Alimentación de forma reproducible desde una base vacía autorizada. | `db/aplicar_migraciones_gtm_alimentacion.sql` |
| revertir_alimentacion_completo | 2026-09-04T12:00:00-05:00 | 15 tablas y schema `[alimentacion]` | Revertir solo el módulo con confirmación, base exacta y huella estructural. | `db/reversiones/revertir_alimentacion_completo.sql` |
| validar_paquete_alimentacion | 2026-09-04T12:00:00-05:00 | Paquete DB `001..012` y `017` | Validar estáticamente secuencia, ownership, FKs, consolidado, rollback y documentación. | `db/validar_paquete_alimentacion.ps1` |
...
- `007`: se retiraron `DiaCicloHorario`, `TramoHorario` y
  ...
  y su vigencia.
- Por alcance posterior del usuario se retiraron completamente las migraciones
  `009` y `010`, así como todos los disparadores. La secuencia inicial termina
  en `008` y crea solo schemas, tablas, restricciones e índices.
...
## 017_crear_validacion_entrega.sql — 2026-09-04
...
- Ruta: `db/migrations/alimentacion/017_crear_validacion_entrega.sql`.
